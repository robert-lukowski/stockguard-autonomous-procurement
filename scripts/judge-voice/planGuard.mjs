...
export function evaluate(planName, plan) {
  ...
  if (planName === "stage-a") {
    const created = judgeFlow.filter(isCreated);
    if (created.length > 0) {
      refuse(
        "the Stage A plan creates the judge contact flow. " +
          "connect_judge_flow_enabled should be false here: creating the flow " +
          "now would race the manual Lex association and fail half-way.",
      );
    } else {
      note("no judge contact flow is created, as Stage A requires");
    }
  } else if (planName === "stage-b") {
    if (judgeFlow.some(isCreated)) {
      note("the judge contact flow is created");
    } else {
      refuse(
        "the Stage B plan does not create the judge contact flow. " +
